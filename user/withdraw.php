...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_withdrawal'])) {
    ...
    if (empty($address)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                INSERT INTO withdrawal_requests (id, user_id, amount, fee_amount, net_amount, withdrawal_method, withdrawal_address, status, requested_at) 
                VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', NOW())
            ");
            $withdrawal_id = generateUUID();
            $stmt->execute([$withdrawal_id, $user_id, $amount, $fee_amount, $net_amount, $valid_method, $address]);
            
            // Get the withdrawal ID for display
            $display_id = substr($withdrawal_id, 0, 8);
            ...
            try {
                ...
                $notifications->createNotification(
                    ...
                    "Your withdrawal request for $" . number_format($amount, 2) . " has been submitted and is being processed.",
                    'info',
                    true // Send email
                );
            } catch (Exception $e) {
            ...
            $success = "Withdrawal request submitted successfully! Request ID: #" . $display_id;
                    ...
                    <form method="POST" id="withdrawalForm">
                        <div class="form-group">
                            ...
                            <select name="withdrawal_method" required>
                                <option value="plisio">Cryptocurrency (USDT, BTC, ETH, LTC, etc.)</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <small style="color: #666; font-size: 0.8rem;">
                                Enter your USDT (TRC20), Bitcoin, or other cryptocurrency wallet address
                            </small>
                        </div>
